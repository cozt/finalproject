...
void para_lower(para* p) {
  if (p == NULL) { return; }
  for (int i = p->start; i <= p->stop && i != p->filesize; ++i) {
    for (int j = 0; j < strlen(p->firstline); ++j){
      p->firstline[j] = tolower(p->firstline[j]);
    }
      for (int j = 0; j < strlen(p->secondline); ++j){
      p->secondline[j] = tolower(p->secondline[j]);
    }
  }
}
